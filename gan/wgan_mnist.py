...
class GAN():
    def __init__(self, args):
        ...
        self.clip_value = float(args.clip_value)
        self.rmsprop_lr = float(args.rmsprop_lr)
        self.nepochs_max = 20000
        self.nepochs_dump_pred_metrics = 250
        self.nepochs_dump_samples = 250 # make this a multiple of the above
        # self.nepochs_dump_pred_metrics = 50
        # self.nepochs_dump_samples = 100 # make this a multiple of the above
        ...
        optimizer_d = RMSprop(lr=self.rmsprop_lr)
        # optimizer_g = Adadelta()
        # optimizer_g = "Adam"
        optimizer_g = RMSprop(lr=self.rmsprop_lr)
        # optimizer_d = "RMSprop"
        # optimizer_g = "RMSprop"
    ...
    def build_discriminator(self):
        ...
        model = Sequential()
        weight_init = RandomNormal(mean=0., stddev=self.clip_value/2)
    ...
    def clip_d_weights(self):
        ...
        weights = [np.clip(w, -self.clip_value, self.clip_value) for w in self.discriminator.get_weights()]
    ...
    def train(self):
        ...
        for epoch in range(self.nepochs_max):
            ...
            for icritic in range(ncritic):
                ...
                # print "real",self.discriminator.predict(imgs)[:10]
                # print "fake",self.discriminator.predict(gen_imgs)[:10]
            ...
            if epoch % self.nepochs_dump_pred_metrics == 0 and epoch > 0:
                ...
                if "epoch" not in self.d_epochinfo: 
                    ...
                    self.d_epochinfo["d_loss"].append(1.-d_loss[0])
                    self.d_epochinfo["g_loss"].append(1.-g_loss)
            ...
            if epoch % self.nepochs_dump_samples == 0 and epoch > 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--clip_value", help="discriminator weight clip magnitude", default=0.01)
    # parser.add_argument("--rmsprop_lr", help="RMSprop learning rate", default=0.0001)
    parser.add_argument("--rmsprop_lr", help="RMSprop learning rate", default=0.001)
