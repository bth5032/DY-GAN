...
class GAN():
    ...
    def build_discriminator(self):
        ...
        #model.add(Lambda(lambda x: K.concatenate([x,getMassFrom4Vecs(x)]), input_shape=output_shape))
        #model.add(Lambda(lambda x: K.concatenate([x,[K.sqrt((x[:,0]+x[:,4])**2 - (x[:,1]+x[:,5])**2 - (x[:,2]+x[:,6])**2 - (x[:,3]+x[:,7])**2)]]), input_shape=output_shape))
        #model.add(LeakyReLU(alpha=0.2))
        model.add(Dense(128))
        model.add(LeakyReLU(alpha=0.2))
